package br.com.sysdesc.boletos.service.boleto;
...
import br.com.sysdesc.boletos.repository.model.BoletoDadosPagamento;
...
import br.com.sysdesc.boletos.service.factory.BancoBoletoFactory;
...
public class BoletoService {
	...
	public br.com.sysdesc.boletos.repository.model.Boleto gerarBoletoComAceite(BoletoDadosCliente cliente,
			BoletoDadosSacadorAvalista avalista, PagamentoBoletoVO pagamentoBoleto, Banco banco,
			TipoTituloEnum tipoTitulo) {

		return gerarBoleto(cliente, avalista, pagamentoBoleto, banco, tipoTitulo, Aceite.A);
	}

	public br.com.sysdesc.boletos.repository.model.Boleto gerarBoletoSemAceite(BoletoDadosCliente cliente,
			BoletoDadosSacadorAvalista avalista, PagamentoBoletoVO pagamentoBoleto, Banco banco,
			TipoTituloEnum tipoTitulo) {
	...
	private br.com.sysdesc.boletos.repository.model.Boleto gerarBoleto(BoletoDadosCliente cliente,
			BoletoDadosSacadorAvalista avalista, PagamentoBoletoVO pagamentoBoleto, Banco banco,
			TipoTituloEnum tipoTitulo, Aceite aceite) {
		...
		Titulo titulo = this.gerarTitulo(bancoGeracao, pagamentoBoleto, tipoTitulo, aceite, this.gerarSacado(cliente),
				nossoNumero, dataDocumento);
		...
		boleto.addTextosExtras("txtRsAgenciaCodigoCedente",
				numeroAgencia + "-" + digitoAgencia + " / " + numeroConta + "-" + digitoConta);
		...
		boleto.setInstrucao2(
				montarJurosMora(configuracao.getCodigoJurosMora(), valor, configuracao.getValorJurosMora()));
		...
		br.com.sysdesc.boletos.repository.model.Boleto modeloBoleto = new br.com.sysdesc.boletos.repository.model.Boleto();
		...
		modeloBoleto.setBoletoDadosPagamento(
				this.montarDadosPagamento(configuracao, diaMaximoPagamento, pagamentoBoleto.getDataVencimento()));
	...
	private BoletoDadosPagamento montarDadosPagamento(ConfiguracaoBoleto configuracao, Date diaMaximoPagamento,
			Date dataVencimento) {
		...
		boletoDadosPagamento
				.setDiasProtesto(codigoProtesto.equals(DIAS_CORRIDOS) ? configuracao.getDiasProtesto() : 0L);
	...
	private Titulo gerarTitulo(BancoBoletoService bancoGeracao, PagamentoBoletoVO pagamentoBoleto,
			TipoTituloEnum tipoTitulo, Aceite aceite, Sacado sacado, String nossoNumero, Date dataDocumento) {
	...
	public void salvar(List<br.com.sysdesc.boletos.repository.model.Boleto> boletos) {
